...
from src.utils.main_utils import save_object, save_numpy_array_data, read_yaml_file, applysmote
...
class DataTransformation:
    ...
    def drop_column(self, df):
        ...
        logging.info("Dropping 'id' and 'postal_code' column")
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        ...
        try:
            ...
            train_df = self.drop_column(train_df)
            train_df = self.convert_credit_score(train_df)

            test_df = self.drop_column(test_df)
            test_df = self.convert_credit_score(test_df)
            ...
            transformed_train_csv_path = os.path.join(
                os.path.dirname(
                    self.data_transformation_config.transformed_train_file_path
                ),
                "transformed_train.csv",
            )
            transformed_test_csv_path = os.path.join(
                os.path.dirname(
                    self.data_transformation_config.transformed_test_file_path
                ),
            ...
            logging.info(f"{train_transformed_df['outcome'].value_counts()}")
            
            logging.info(f"Transformed test CSV saved at: {transformed_test_csv_path}")
            logging.info(f"{test_transformed_df['outcome'].value_counts()}")
